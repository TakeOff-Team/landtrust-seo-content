...
- **2026-05-03: ~10-fire W17b loop — local `routine_prompt.md` was updated, the cron was not.** Same shape as the W17a loop, different root cause. Routine v3 (commit 119a4b4) updated `routine_prompt.md` in the repo to use the PR + `gh pr merge --auto` flow, but **never pushed the new prompt to the cron config on claude.ai**. The cron kept executing the v1 prompt (`git push origin main`), which is blocked by branch protection. So every hourly fire wrote W17b end-to-end on a sandbox branch, failed the push, the session ended, work was lost — same loop as W17a, with W17b. Diagnosis path: `RemoteTrigger get trig_018namShCPJmKCvKnn3HxAug` and check `events[0].data.message.content` against local `routine_prompt.md` — if they differ, that's the bug. **Fix:** ran `RemoteTrigger update` with the v3 prompt content. The body shape needs `events[0].data` containing `{uuid, session_id: "", type: "user", parent_tool_use_id: null, message: {role: "user", content: <prompt>}}` — `type` and `parent_tool_use_id` go INSIDE `data`, not at the event level. The API errors are misleading ("event_type required" then "event_type unknown"); use the canonical shape from the /schedule skill's SKILL.md. Banner added to `routine_prompt.md` so future edits trigger the cron sync. No backlog state was corrupted — W17b row remained `pending`, so the next correct fire just picks it up normally.
...
| Symptom | Fix |
|---|---|
| Briefs are thin / off-voice | `RemoteTrigger update` the routine, change `job_config.ccr.session_context.model` to `claude-opus-4-7`. Or sharpen `routine_prompt.md` and update the routine's `events[0].data.message.content` (see next row — the local file is documentation only). |
| Edited `routine_prompt.md` and want the change live | Local `routine_prompt.md` is documentation; the cron stores its own copy. After editing, push the new content via `RemoteTrigger action=update trigger_id=trig_018namShCPJmKCvKnn3HxAug` with body `{job_config: {ccr: {environment_id: "env_01Q28jtqXq8ddQddJjZUh6aV", events: [{data: {uuid: <fresh v4>, session_id: "", type: "user", parent_tool_use_id: null, message: {role: "user", content: <prompt>}}}]}}}`. Verify with `RemoteTrigger get` that `events[0].data.message.content` matches. The 2026-05-03 W17b loop happened because this step was skipped. |
